<template>
  <page-layout :menu="isCreator(recipe)">
    ...
    <div v-if="recipe" class="text-center">
      <recipe-card-image :width="$device.isMobile ? '100%' : ''" :recipe="recipe" @toggle-favourite="toggleFavourite" />
      <a-title>
        {{ recipe.name }}
      </a-title>
      ...
      <div
        class="d-flex align-stretch justify-center my-2"
      >
        <span v-if="recipe.duration" class="mr-3"><a-icon name="fa-solid fa-clock" /> {{ recipe.duration }} {{ $t('min') }}</span>
        <v-divider
      ...
      <a-tabs :items="items">
        <a-tabs-item key="ingredients">
          <div class="my-3">
            <p v-for="ingredient in recipe.ingredients" :key="ingredient">
              <a-icon name="fa-solid fa-square-check" class="mr-2" />{{ ingredient }}
            </p>
          </div>
        </a-tabs-item>
      ...
      <a-modal name="delete-recipe" @accept="onAcceptDelete">
        ...
        <p>
          {{ $t('delete_recipe_text') }} <span class="font-weight-bold">{{ recipe.name }}</span> ?
        </p>
      </a-modal>
    </div>
  </page-layout>
...
export default {
  ...
  methods: {
    onEdit () {
      this.$router.push({ name: 'recipes-edit-id', params: { id: this.recipeId } })
    },
    onDelete () {
