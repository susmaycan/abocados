...
class AuthTestCase(TestCase):
    ...
    def test_signup_empty_fields(self):
        ...
        self.assertEqual(response.content['password'][0], 'This field may not be blank.')
        self.assertEqual(response.content['username'][0], 'This field may not be blank.')
    ...
    def test_signup_wrong_password_min_length(self):
        ...
        self.assertEqual(response.status_code, HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content['password'][0], 'Ensure this field has at least 8 characters.')
    ...
    def test_signup_wrong_password_max_length(self):
        ...
        self.assertEqual(response.status_code, HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content['password'][0], 'Ensure this field has no more than 64 characters.')
    ...
    def test_get_authenticated_user_ok(self):
        response = self.make_authenticated_call(
    ...
    def test_validate_email_empty_token(self):
        ...
        response = self.make_validate_email_call(
            data=body
        )
        self.assertEqual(response.status_code, HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content['non_field_errors'][0], 'Token is invalid or expired')
    ...
    def test_validate_email_already_active(self):
        ...
        self.assertEqual(response.status_code, HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content['non_field_errors'][0], 'Email has already been verified')
    ...
    def test_request_password_recovery_not_registered_email(self):
        ...
        response = self.make_password_recovery_request(
            data=body
        )
        self.assertEqual(response.status_code, HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content['non_field_errors'][0], 'Email not registered')
    ...
    def test_check_password_recovery_empty_token(self):
        ...
        response = self.make_password_recovery_check(
            data=body
        )
        self.assertEqual(response.status_code, HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content['non_field_errors'][0], 'Token is invalid or expired')
    ...
    def test_confirm_password_recovery_empty_token(self):
        ...
        response = self.make_password_recovery_confirm(
            data=body
        )
        self.assertEqual(response.status_code, HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content['non_field_errors'][0], 'Token is invalid or expired')
