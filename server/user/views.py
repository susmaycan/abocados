...
from user.serializers import (
    ...
    UserRetrieveSerializer, ResetPasswordEmailRequestSerializer, ActivateAccountSerializer, PasswordRecoveryCheckSerializer, PasswordRecoveryConfirmSerializer, FavouriteRecipeSerializer
)
...
def send_password_reset_email(user):
    ...
    EmailUtils.send(
        subject=_('Reset Password Abocados'),
        template=template,
        to=[user.email]
    )
...
class AuthViewSet(GenericViewSet):
    ...
    def request_validation_email(self, request):
        ...
        try:
            ...
            if user.is_active:
                return Response(_('Email is already verified'), status=HTTP_400_BAD_REQUEST)
    ...
    def activate_account(self, request):
        ...
        if not default_token_generator.check_token(user, token):
            return Response(_('Token is invalid or expired'), status=HTTP_400_BAD_REQUEST)
...
class UserViewSet(GenericViewSet, EnablePartialUpdateMixin, UpdateModelMixin, DestroyModelMixin, RetrieveModelMixin):
    queryset = User.objects.filter(is_active=True)
    boo = 4

    def get_permissions(self):
        permission_classes = [IsAuthenticated, IsStandardUser]
        if self.action in [RestFrameworkActions.UPDATE, RestFrameworkActions.PARTIAL_UPDATE, RestFrameworkActions.DESTROY]:
...
class UserFavouriteRecipesViewSet(ListModelMixin, GenericViewSet):
    ...
    def add(self, request, *args, **kwargs):
        ...
        if recipe.creator.id == user.id:
            return Response(_('Recipe cannot be created by user'), status=HTTP_400_BAD_REQUEST)

        if user.saved_recipes.filter(id=recipe.id).exists():
            return Response(_('Recipe is already favourited'), status=HTTP_400_BAD_REQUEST)
