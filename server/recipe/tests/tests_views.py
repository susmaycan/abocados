...
class RecipeTestCase(TestCase):
    ...
    def _get_recipe_request(self, name=None, directions=None, rating=None, duration=None, ingredients=None, categories=None):
    ...
    def test_retrieve_recipe_list_filter_to_from(self):
        ...
        query_param = {
            ...
            'to_date': datetime.datetime(2020, 5, 18).strftime(settings.DATETIME_FORMAT)
        }
        response = self.make_retrieve_recipe_list_call(
            user=self.user1,
            query_params=query_param
    ...
    def test_retrieve_recipe_list_filter_category(self):
        ...
        self.assertEqual(response.content['results'][0]['categories'][0]['name'], self.category1.name)
        self.assertEqual(response.content['results'][0]['categories'][0]['id'], self.category1.id)
    ...
    def test_retrieve_recipe_by_id_other_creator(self):
        ...
        response = self.make_retrieve_recipe_call(
            uuid=recipe_by_other_creator.id,
            user=self.user1
        )
        self.assertEqual(response.status_code, HTTP_200_OK)
    ...
    def test_update_recipe_ok(self):
        ...
        self.assertEqual(content['creator']['username'], original_recipe.creator.username)
        self.assertEqual(sorted(content['ingredients']), sorted(original_recipe.ingredients))
    ...
    def test_update_recipe_readonly_fields(self):
        ...
        self.assertEqual(content['creator']['id'], original_recipe.creator.id)
        self.assertEqual(content['creator']['username'], original_recipe.creator.username)
        self.assertEqual(content['created_at'], original_recipe.created_at.strftime(settings.DATETIME_FORMAT))
